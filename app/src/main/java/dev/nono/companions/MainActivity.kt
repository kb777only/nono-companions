...
class MainActivity : Activity() {
    ...
    private fun render() {
        ...
        text("Version ${packageManager.getPackageInfo(packageName,0).versionName}",14f)
        ...
        text(AccessibilitySetup.status(this),15f)
        text("Cherchez « NoNo · Détection du clavier » dans Accessibilité. Si Android bloque les paramètres restreints après installation, ouvrez les informations de l’application pour vérifier cette autorisation. NoNo ne peut pas accorder cet accès lui-même.",14f)
        button("Détection automatique du clavier · Accessibilité") {
            AlertDialog.Builder(this).setTitle("Local, optional keyboard awareness").setMessage(getString(R.string.context_description)).setNegativeButton("Not now",null).setPositiveButton("Open Android settings") { _,_ -> AccessibilitySetup.open(this) }.show()
        }
        ...
        button("Informations Android de NoNo") { startActivity(Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS,Uri.parse("package:$packageName"))) }
