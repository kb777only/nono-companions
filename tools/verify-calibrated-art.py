...
v11=any(r['asset'].startswith('w11-') for r in rows)
weather=v11 or any(r['asset'].startswith('w10-') for r in rows)
landmarks=json.loads((ROOT/('art/weather-v11/published.json' if v11 else 'art/weather-v10/published.json' if weather else 'art/v2/isolated-landmarks.json')).read_text(encoding='utf-8'))
...
assert len(rows)==(910 if weather else 296) and len(images)==(75 if v11 else 33 if weather else 19)
report=dict(version='0.11.0' if v11 else '0.10.0' if weather else '0.9.1',frames=len(rows),atlases=len(images),failures=failures,
...
(ROOT/('docs/scale-audit/weather-v11-verification.json' if v11 else 'docs/scale-audit/weather-verification.json' if weather else 'docs/scale-audit/calibrated-verification.json')).write_text(json.dumps(report,indent=2),encoding='utf-8')
