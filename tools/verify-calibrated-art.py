...
landmarks=json.loads((ROOT/'art/v2/isolated-landmarks.json').read_text(encoding='utf-8'))
...
assert len(rows)==296 and len(images)==19
report=dict(version='0.9.1',frames=len(rows),atlases=len(images),failures=failures,
...
(ROOT/'docs/scale-audit/calibrated-verification.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
